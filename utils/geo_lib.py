'''
https://github.com/FreyrS/dMaSIF/blob/master/geometry_processing.py
'''
...
from utils.geo_helper import *


def project_iface_labels(surf_xyz, surf_batch, mesh_xyz, mesh_labels, mesh_batch, threshold=2.0):
    # https://github.com/FreyrS/dMaSIF/blob/master/data_iteration.py#L101

    queries = surf_xyz # P["xyz"]
    batch_queries = surf_batch # P["batch"]
    source = mesh_xyz # P["mesh_xyz"]
    batch_source = mesh_batch # P["mesh_batch"]
    labels = mesh_labels # P["mesh_labels"]
    ...
    nn_dist_i = (
        D_ij.min(dim=1).view(-1, 1) < threshold # find the nearest neighbor, if the distance is smaller than a given threshold, than ...
...
def soft_distances(x, y, batch_x, batch_y, smoothness=0.01, atomtypes=None):
    ...
    if atomtypes is not None:
        ...
        atomic_radii = atomic_radii / atomic_radii.min() # normlized radius
...
def curvatures(
    ...
    for s, scale in enumerate(scales):
        ...
        # Gaussian window:
        window_ij = (-d2_ij / (2 * (scale ** 2))).exp()  # (N, N, 1) scale: sigma
